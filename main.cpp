...
int main(void) {  
    char mode;
    std::cout << "Elige un punto de partida(M)/cualquier punto(cualquiera): ";
    std::cin >> mode;
    if (mode == 'M' || mode == 'm') {
        std::cout << "Elige el punto de posición entre -20 y 61: " << std::endl;
    ...
    window = glfwCreateWindow(800, 800, "Cohete contra asteroide", NULL, NULL);
    nn_window = glfwCreateWindow(400, 400, "Estructura red neuronal", NULL, window);
    if (!window) {
        glfwTerminate();
        return -1;
    }
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (players_finished >= 20) {
            std::cout << std::endl << "INFO: Generación número (" << generation << ") comienza" << std::endl;

            for (int i = 0; i < species; i++) {
                std::cout << i << " Puntuación: " << Rockets[i].score << std::endl;
            }

            // Half well performed rockets
            std::cout << "INFO: Proceso de evolución" << std::endl;
            ...
            if (new_best_score >= 9.2) {
                ...
                file << jsonfile;
                file.close();
                std::cout << "INFO: Hecho! se encontró una estructura de 9.2 higher" << std::endl;
                break;
            }
